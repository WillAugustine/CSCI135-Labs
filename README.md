
## Lab Instructions:
- [All Labs](LabInstructions)
- Lab 1: [Hello Python! Hello World](LabInstructions/CSCI135FundamentalsOfCS2023-08-23_25-Assignment01-HelloWorld.pdf)
- Lab 2: [Hello Arguments](LabInstructions/CSCI135FundamentalsOfCS2023-08-30_09-01-Assignment02-HelloArguments.pdf)
- Lab 3: [Command Line Utilities](LabInstructions/CSCI135FundamentalsOfCS2023-09-06_08-Assignment03-CmdLineUtilities.pdf)
- Lab 4: [Conditionals](LabInstructions/CSCI135FundamentalsOfCS2023-09-13_15-Assignment04-Conditionals.pdf)
- Lab 5: [Strings and Loops](LabInstructions/CSCI135FundamentalsOfCS2023-09-20_22-Assignment05-TextLoops.pdf)

## Lab Solutions:
- Lab 1 - Hello Python! Hello World:
  - [HelloWorld.py](01-Hello_Python_Hello_World/HelloWorld.py)
  - [GreetingsFromMe.py](01-Hello_Python_Hello_World/GreetingsFromMe.py)

- Lab 2 - Hello Arguments:
  - [HelloArgs.py](02-Hello_Arguments/HelloArgs.py)
  - [Stairway.py](02-Hello_Arguments/Stairway.py)

- Lab 3 - Command Line Utilities:
  - [DecimalToBinary.py](03-Command_Line_Utilities/DecimalToBinary.py)
  - [BinaryToDecimal.py](03-Command_Line_Utilities/BinaryToDecimal.py)
  - [DecimalToHex.py](03-Command_Line_Utilities/DecimalToHex.py)
  - [HexToDecimal.py](03-Command_Line_Utilities/HexToDecimal.py)
 
- Lab 4 - Conditionals:
  - [Shipping.py](04-Conditionals/Shipping.py)
  - [Lottery.py](04-Conditionals/Lottery.py)
  - [Zodiac.py](04-Conditionals/Zodiac.py)
 
- Lab 5 - Strings and Loops:
  - [PickTeams.py](05-Strings_and_Loops/PickTeams.py)
